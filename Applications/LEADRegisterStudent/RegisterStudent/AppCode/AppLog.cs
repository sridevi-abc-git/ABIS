...
namespace RegisterStudent.AppCode
{
	public class AppLog
	{
		...
		static public void Write(Exception e, MessageType type, string connectionString)
		{
			AppLog						logWorker;
			string						logging;
			bool						log			= false;
			...
			try
			{
				...
				if (log)
				{
					...
					logWorker = new AppLog(e, type, connectionString);
		...
		public AppLog(Exception ex, MessageType type, string connectionString)
		{
		...
		protected void SendEmail(string buffer)
		{
			...
			try
			{
				...
				conn = new OracleConnection(m_connectionString); // Util.Decrypt(connectionString));
				...
				while ((retry < 3) && (conn.State != System.Data.ConnectionState.Open))
				{
					...
					{
						if (retry == 0) AppCode.AppLog.Write(ex, AppCode.AppLog.MessageType.ERROR, m_connectionString);
					}
